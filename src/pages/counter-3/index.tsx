...
export default () => {
    ...
    let multiplier: string = useProps((props): string => String(props.multiplier_3));
    let counter: number = useProps((props): number => Number(props.counter_3));
    let statusCounter: boolean = useProps((props): boolean => Boolean(props.status_counter_3));
    let [nameCounter, setNameCounter] = React.useState(counterNames.split(';')[2]);
    ...
    function saveCounter(event: any): void|false
    {
        ...
        arr.splice(2, 1, event.detail.value.name);
        str = arr.join(';');
        ...
        ACTIONS.multiplier_3.set(event.detail.value.radio);
        ACTIONS.status_counter_3.set(Boolean(event.detail.value.status));
